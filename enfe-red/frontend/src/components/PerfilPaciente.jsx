...
const PerfilPaciente = () => {
  ...
  if (!datosPaciente && !error) return (
    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '100px', fontFamily: "'Segoe UI', Roboto, sans-serif" }}>
      <h3 style={{ color: '#3498db', fontWeight: '600' }}>⏳ Cargando tu perfil...</h3>
    </div>
  );

  if (error && !datosPaciente) return (
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
          <h2 style={{ color: '#2c3e50', margin: 0, fontSize: '28px', fontWeight: '800' }}>
            Mi <span style={{ color: '#3498db' }}>Perfil</span>
          </h2>
          <button 
          ...
          {editando ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <label style={{ marginBottom: '6px', fontWeight: '600', color: '#475569', fontSize: '14px' }}>Teléfono</label>
                <input name="telefono" value={form.telefono || ""} onChange={handleChange} placeholder="Ej: 2614567890" style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', color: '#1e293b', outline: 'none' }} />
              </div>

              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <label style={{ marginBottom: '6px', fontWeight: '600', color: '#475569', fontSize: '14px' }}>Dirección</label>
                <input name="direccion" value={form.direccion || ""} onChange={handleChange} placeholder="Calle, Número, Departamento" style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', color: '#1e293b', outline: 'none' }} />
              </div>

              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <label style={{ marginBottom: '6px', fontWeight: '600', color: '#475569', fontSize: '14px' }}>Historial Médico</label>
                <textarea name="historial_medico" value={form.historial_medico || ""} onChange={handleChange} rows="3" placeholder="Alergias, patologías previas..." style={{ padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', color: '#1e293b', outline: 'none', resize: 'vertical' }} />
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '25px', marginBottom: '30px' }}>
                <div style={{ width: '80px', height: '80px', backgroundColor: '#e0f2fe', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#0284c7', fontSize: '28px', fontWeight: '800' }}>
                  {datosPaciente.nombre?.charAt(0)}{datosPaciente.apellido?.charAt(0)}
                </div>
                <div>
              ...
              <div style={{ backgroundColor: '#f8fafc', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '25px' }}>
                <h3 style={{ margin: '0 0 15px 0', color: '#334155', fontSize: '18px' }}>Tus Datos de Contacto y Salud</h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0', paddingBottom: '10px' }}>
                    ...
                    <span style={{ color: '#1e293b', fontWeight: '700' }}>{datosPaciente.telefono || "No registrado"}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0', paddingBottom: '10px' }}>
                    <span style={{ color: '#64748b', fontWeight: '600' }}>Dirección:</span>
                    <span style={{ color: '#1e293b', fontWeight: '700' }}>{datosPaciente.direccion || "No registrada"}</span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                    <span style={{ color: '#64748b', fontWeight: '600' }}>Historial Médico:</span>
                    <p style={{ color: '#1e293b', margin: 0, backgroundColor: '#ffffff', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '14px', lineHeight: '1.5' }}>
                      {datosPaciente.historial_medico || "Sin historial previo registrado."}
                    </p>
                  </div>
                </div>
              </div>
